		...
		resolve: {
			"currentAuth":['authService', function(authService){
				 return authService.auth().$requireSignIn();
				  //return auth.auth().$waitForSignIn();
			}],
			userTodos : function(){	
				return {
					somedata: false
...
.controller('todoController', ['$timeout', '$log', 'moment','todoService','authService','$mdToast','toastService','currentAuth', function($timeout,$log,moment,todoService,authService,$mdToast,toastService,currentAuth){
	var todo = this;
	...
	toastService.showSimpleToast('worked',3000);
